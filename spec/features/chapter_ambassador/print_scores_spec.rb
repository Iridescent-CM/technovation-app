...
RSpec.feature "Chapter ambassador prints event scores" do
  scenario "assigned judges and teams are grouped together" do
    ...
    event = chapter_ambassador.regional_pitch_events.create!(FactoryBot.attributes_for(:event, :senior))
    ...
    visit chapter_ambassador_printable_score_path(event)
  ...
  scenario "other event scores are filtered out when assigned judges are in multiple events" do
    chapter_ambassador = FactoryBot.create(:chapter_ambassador, :approved)
    event_a = chapter_ambassador.regional_pitch_events.create!(FactoryBot.attributes_for(:event, :senior))
    event_b = chapter_ambassador.regional_pitch_events.create!(FactoryBot.attributes_for(:event, :senior))
    ...
    sign_in(chapter_ambassador)
    visit chapter_ambassador_printable_score_path(event_a)
