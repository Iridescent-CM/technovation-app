...
RSpec.describe TeamSubmission do
  ...
  describe "#percent_complete" do
    ...
    it "returns 13 for one junior team item completed" do
      submission = FactoryBot.create(:submission, :junior)
      submission.update(app_name: "Something")
      expect(submission.percent_complete).to eq(13)
    end

    it "returns 11 for one senior team item completed" do
      submission = FactoryBot.create(:submission, :senior)
      submission.update(app_name: "Something")
      expect(submission.reload.percent_complete).to eq(11)
    end

    it "returns 88 percent for all of the junior items completed" do
    ...
    it "returns 89 percent for all of the senior items completed" do
      submission = FactoryBot.create(:submission, :senior, :complete)

      submission.update_column(:source_code, "something.zip")
      submission.update_column(:published_at, nil)
      submission.touch
      expect(submission.reload.percent_complete).to eq(89)
    end
