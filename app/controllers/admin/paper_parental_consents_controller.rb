module Admin
  class PaperParentalConsentsController < AdminController
    def create
      ...
      sql = ActiveRecord::Base.send(:sanitize_sql_array, ["INSERT INTO parental_consents (student_profile_id, electronic_signature, created_at, updated_at)
                                                                  VALUES (?, ?, ?, ?)", student.id, 'ON FILE', Time.current, Time.current])
