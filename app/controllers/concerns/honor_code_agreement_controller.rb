module HonorCodeAgreementController
  ...
  def create
    @honor_code_agreement = current_account.build_honor_code_agreement(honor_code_agreement_params)

    if @honor_code_agreement.save
      flash.now[:success] = "Thank you for your promise. Now get out there and show us what you got!"
      redirect_to cookies.delete(:redirected_from) || send("#{current_account.type_name}_dashboard_path")
    else
      render template: 'honor_code_agreements/new'
    end
  end
