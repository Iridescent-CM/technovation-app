...
export default function Attendee(json) {
  ...
  function notifyRemoteOfAssignmentChange(opts) {
    const url = "/chapter_ambassador/judge_assignments";
  ...
  this.assignmentSaved = () => {
    ...
    if (this.recentlyAdded) this.recentlyAdded = false;
  };
  ...
  this.matchesQuery = (query) => {
    ...
    if (this.scope === "Team") {
      return this.name.match(pattern) || this.submission.name.match(pattern);
    } else {
