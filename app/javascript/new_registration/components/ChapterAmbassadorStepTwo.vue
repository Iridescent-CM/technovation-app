    ...
    <div class="form-wrapper">
      <div id="step-two-chapter-ambassador">
        ...
        <FormulateInput
          ...
          :validation-messages="{
            required:
              'You must be 18 years or older in order to become a Chapter Ambassador',
          }"
          @keyup="checkValidation"
          @blur="checkValidation"
  ...
  methods: {
    async getOrganizationName() {
      try {
        const response = await axios.get(
          "/api/registration/chapter_organization_name",
          {
    ...
    checkValidation() {
      ...
      if (
        ...
        document.getElementById("chapterAmbassadorJobTitle").value.length ===
          0 ||
