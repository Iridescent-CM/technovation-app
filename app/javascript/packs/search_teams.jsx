...
class TeamSearch extends Component {
  ...
  handleChange = (e) => {
    this.setState({
      should_search: e.target.value.length >= 3,
      performed_search: false,
  ...
  renderTeamList() {
    if (this.state.matched_teams.length > 1)
      return(
        <div>
          <p>We found {this.state.matched_teams.length} results!</p>
    ...
    if (this.state.matched_teams.length > 0) {
      return(
        <div>
          <p>We found {this.state.matched_teams.length} result!</p>
          <TeamList teams={this.state.matched_teams} />
        </div>
      );
    } else if (this.state.performed_search) {
      return(
        <div>
          <p>We couldn't find a team with that name!</p>
          <a href={"/team_registrations/new?name=" + this.state.query}>Register {this.state.query} now</a>
        </div>
      );
    }
  }
